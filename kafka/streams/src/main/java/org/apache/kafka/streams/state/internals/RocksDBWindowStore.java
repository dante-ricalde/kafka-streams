...
public class RocksDBWindowStore<K, V> extends WrappedStateStore.AbstractStateStore implements
    WindowStore<K, V> {
    ...
    public void init(final ProcessorContext context, final StateStore root) {
        ...
        // construct the serde
        serdes = new StateSerdes<>(ProcessorStateManager
            .storeChangelogTopic(context.applicationId(), bytesStore.name()),
    ...
    public V fetch(final K key, final long timestamp) {
        final byte[] bytesValue = bytesStore.get(WindowKeySchema
            .toStoreKeyBinary(key, timestamp, seqnum, serdes));
        if (bytesValue == null) {
            return null;
        }
    ...
    public KeyValueIterator<Windowed<K>, V> fetch(final K from, final K to, final long timeFrom, final long timeTo) {
        final KeyValueIterator<Bytes, byte[]> bytesIterator = bytesStore.fetch(Bytes.wrap(serdes.rawKey(from)), Bytes
            .wrap(serdes.rawKey(to)), timeFrom, timeTo);
